...
static NSView *make_editor(AudioUnit unit, NSSize preferred)
{
	...
	// nil for the owner: the AU instance owns the engine and the host tears the
	// unit down after the view it was handed, so there is nothing for the view
	// to hold on to. The AUv3 is the one that needs that (panel_nsview.h)
