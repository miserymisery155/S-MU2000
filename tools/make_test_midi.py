...
def case_bendasn():
    """**ベンドの割り当て**（6 つ組）を 1 つずつ振る（6.201）。

    XG はベンドにも「音程・切る高さ・音量・LFO の音程/フィルタ/音量」の
    6 つ組を持っている（+0x23-0x28）。音程以外は今まで firmware に
    丸投げしていた。実機はベンドだけ値の作り方が違って、

      * 切る高さ・音量・LFO の音程 … 中央からの離れ >> 8 を使う
      * LFO のフィルタ・音量       … その**絶対値** × 深さ / 256

    ここが壊れると、ベンドを踏む曲で音色まで変わってしまう。
    """
    ev = head()
    ev += [(1.0, b'\xc0\x30')]                       # Strings（伸びる音）
    ev += note(0, 60, 100, 1.2, 1.0)                 # 1 音目。ここで写し取る
    # --- ベンド → 切る高さ（+0x24）
    ev += [(2.4, xg([0x08, 0x00, 0x24, 0x50]))]
    ev += note(0, 62, 100, 2.6, 1.4)
    ev += [(3.0, b'\xe0\x00\x60')]                  # 上へ
    ev += [(3.6, b'\xe0\x00\x20')]                  # 下へ
    ev += [(4.2, b'\xe0\x00\x40')]
    ev += [(4.3, xg([0x08, 0x00, 0x24, 0x40]))]
    # --- ベンド → 音量（+0x25）
    ev += [(4.6, xg([0x08, 0x00, 0x25, 0x50]))]
    ev += note(0, 64, 100, 4.8, 1.2)
    ev += [(5.2, b'\xe0\x00\x70')]
    ev += [(5.6, b'\xe0\x00\x10')]
    ev += [(6.0, b'\xe0\x00\x40')]
    ev += [(6.1, xg([0x08, 0x00, 0x25, 0x40]))]
    # --- ベンド → LFO の音程（+0x26）
    ev += [(6.4, xg([0x08, 0x00, 0x26, 0x40]))]
    ev += note(0, 65, 100, 6.6, 1.0)
    ev += [(7.0, b'\xe0\x00\x60')]
    ev += [(7.6, b'\xe0\x00\x40')]
    ev += [(7.8, xg([0x08, 0x00, 0x26, 0x00]))]
    # --- ベンド → LFO のフィルタ（+0x27）
    ev += [(8.0, xg([0x08, 0x00, 0x27, 0x40]))]
    ev += note(0, 67, 100, 8.2, 1.0)
    ev += [(8.6, b'\xe0\x00\x60')]
    ev += [(9.2, b'\xe0\x00\x40')]
    ev += [(9.4, xg([0x08, 0x00, 0x27, 0x00]))]
    # --- ベンド → LFO の音量（+0x28）
    ev += [(9.6, xg([0x08, 0x00, 0x28, 0x40]))]
    ev += note(0, 60, 100, 9.8, 1.6)
    ev += [(10.2, b'\xe0\x00\x68')]
    ev += [(11.0, b'\xe0\x00\x40')]
    ev += [(11.4, xg([0x08, 0x00, 0x28, 0x00]))]
    return [track(seq(ev))], 13.0


def case_progrel():
    """**離している最中・ペダルで押さえたままの音色替え**（6.202）。

    `progchg` は鳴っている最中の替えを見ているが、**離しに入った音**と
    **ダンパーで押さえたままの音**、それに**バンク LSB だけ**を替える道は
    どこの試験も通っていなかった。

    実機は鳴っている声（離し中も含む）には手を出さない。こちらは
    パートの塊を読み直す作りなので、離し中の音の減衰や切る高さを
    新しい音色のもので書き直してしまうと、実機に無い変化が出る。
    """
    ev = head()
    ev += [(1.0, b'\xc0\x30')]                       # Strings（伸びる音）
    ev += note(0, 60, 100, 1.2, 1.0)                 # 1 音目。ここで写し取る
    # --- 離している最中に替える（離しの長い音色から短い音色へ）
    ev += note(0, 64, 100, 2.4, 0.4)
    ev += [(3.0, b'\xc0\x18')]                       # 24 Nylon Gtr（まだ離しの最中）
    ev += note(0, 67, 100, 3.4, 0.6)
    ev += [(4.2, b'\xc0\x30')]
    # --- ダンパーで押さえたまま替える
    ev += [(4.6, b'\xb0\x40\x7f')]
    ev += note(0, 60, 100, 4.8, 0.4)
    ev += [(5.4, b'\xc0\x00')]                       # 押さえている音がある所で替える
    ev += note(0, 65, 100, 5.6, 0.4)
    ev += [(6.4, b'\xb0\x40\x00')]                   # まとめて離す
    # --- バンク LSB だけ替える（MSB も番号もそのまま）
    ev += [(7.0, b'\xb0\x00\x00'), (7.02, b'\xb0\x20\x01'),
           (7.04, b'\xc0\x30')]
    ev += note(0, 62, 100, 7.2, 0.8)
    ev += [(8.2, b'\xb0\x20\x00'), (8.22, b'\xc0\x30')]
    ev += note(0, 62, 100, 8.4, 0.8)
...
def case_banklsb():
    ...
    実機はプログラム 48 で LSB 0・1・3・6・8・12・14 しか受けず、
    残りは**丸ごと無視**して前の音色のまま鳴らす。こちらが素直に
    切り替えてしまうと、そこだけ別の音になる。

    受ける LSB と受けない LSB を交互に送って、そのつど鳴らす。
    """
    ev = head()
    ev += [(1.0, b'\xb0\x00\x00'), (1.01, b'\xb0\x20\x00'),
           (1.02, b'\xc0\x30')]
    ev += note(0, 60, 100, 1.2, 0.8)                 # 1 音目。ここで写し取る
    t = 2.4
    for lsb in (2, 1, 4, 3, 5, 6, 7, 0):
        ev += [(t, b'\xb0\x20' + bytes([lsb])), (t + 0.01, b'\xc0\x30')]
...
def case_ctlrest():
    """**まだ 1 度も送っていなかった CC**。試験の MIDI を全部当たって、
    XG が決めている CC のうち触れていないものを出したら 7 つ残っていた。

    * CC68 レガートフットスイッチ（モノのとき、重ねた鍵で鳴らし直さない）
    * CC69 ホールド 2（離しをゆっくりにする。押さえ続けるダンパーとは違う）
    * CC96 / CC97 データインクリメント・デクリメント（RPN の値を 1 ずつ動かす）
    * CC124 / CC125 オムニオフ・オムニオン（実機ではどちらも鍵を離す）

    native はこの 4 つを自分ではさばかないので firmware へ回している。
    回し方が間違っていると、ここで音の長さか高さが変わる。
    """
    ev = head()
    ev += [(0.9, bytes([0xc0, 48]))]                 # Strings（尾が長い）
    # --- CC68 レガート。モノにしてから、鍵を重ねる
    ev += [(1.0, b'\xb0\x7e\x01')]                  # CC126 モノ
    ev += [(1.2, bytes([0x90, 60, 100])), (1.5, bytes([0x90, 64, 100])),
           (1.7, bytes([0x80, 60, 0x40])), (2.0, bytes([0x80, 64, 0x40]))]
    ev += [(2.4, b'\xb0\x44\x7f')]                  # CC68 レガート 入
    ev += [(2.6, bytes([0x90, 60, 100])), (2.9, bytes([0x90, 64, 100])),
           (3.1, bytes([0x80, 60, 0x40])), (3.4, bytes([0x80, 64, 0x40]))]
    ev += [(3.6, b'\xb0\x44\x00'), (3.6, b'\xb0\x7f\x00')]   # 戻す（ポリ）
    # --- CC69 ホールド 2。離しがゆっくりになる
    ev += [(3.9, bytes([0xc0, 0]))]                  # GrandPno
    ev += note(0, 60, 100, 4.1, 0.4)
    ev += [(4.9, b'\xb0\x45\x7f')]                  # CC69 入
    ev += note(0, 62, 100, 5.1, 0.4)
    ev += [(5.9, b'\xb0\x45\x00')]
    # --- CC96 / CC97。RPN 0（ベンドの幅）を 1 ずつ動かしてからベンド
    ev += [(6.1, b'\xb0\x65\x00'), (6.12, b'\xb0\x64\x00')]
    ev += [(6.2 + 0.02 * i, b'\xb0\x60\x00') for i in range(6)]  # +6 半音
    ev += note(0, 64, 100, 6.5, 1.0)
    ev += [(6.8, b'\xe0\x00\x60')]
    ev += [(7.6 + 0.02 * i, b'\xb0\x61\x00') for i in range(4)]  # -4 半音
    ev += [(7.8, b'\xe0\x00\x40')]                  # 中央へ戻す
    ev += note(0, 64, 100, 8.0, 1.0)
    ev += [(8.3, b'\xe0\x00\x60')]
    # --- CC124 / CC125 オムニ
    ev += [(9.0, b'\xe0\x00\x40')]
    ev += [(9.2, bytes([0x90, 60, 100])), (9.2, bytes([0x90, 67, 100]))]
    ev += [(9.7, b'\xb0\x7c\x00')]                  # CC124 オムニオフ
    ev += [(10.2, bytes([0x90, 62, 100])), (10.2, bytes([0x90, 69, 100]))]
    ev += [(10.7, b'\xb0\x7d\x00')]                 # CC125 オムニオン
    return [track(seq(ev))], 12.0


def case_xgpeg():
    """**音程の包絡線の離し**（XG の `08 pp 6B`・`6C` ＝ ワーク RAM の
    +0x64・+0x65）。

    `peg_release()`（6.205）はこの 2 つを読んでいるのに、試験が
    CC でも SysEx でも 1 度も触っていなかった。番地を機械的に数えて
    見つけた（6.149 と同じやり方）。

    * `6C`（+0x65）が 64 より大きいと、離しの速さが表（`PEG_REL_TAB`）で
      頭打ちになる。小さいと `素 - (差 >> 1)` で遅くなる
    * `6B`（+0x64）が中央でなければ、行き先が同じでも書き直す

    バンク 0 で音程の包絡線の離しが動くのは Rain（96）だけ。
    """
    ev = head()
    ev += [(1.0, bytes([0xc0, 96]))]                 # Rain
    ev += note(0, 60, 100, 1.2, 0.6)                  # 1 音目。ここで写し取る
    t = 2.4
    for lo, val in ((0x6c, 100), (0x6c, 30), (0x6c, 64),
                    (0x6b, 100), (0x6b, 30), (0x6b, 64)):
        ev += [(t, xg([0x08, 0x00, lo, val]))]
        ev += note(0, 62, 100, t + 0.2, 0.6)
        t += 1.6
    return [track(seq(ev))], t + 1.0
...
def case_xgsys():
    ...
    どれも CC で試験しているが、**SysEx の道は 1 度も通していなかった**。

    実機は同じ番地でも**書き方で結果が変わることがある**（ドラムの
    立ち上がりは NRPN で書いたときだけ計算し直す。6.180）ので、
    別の軸として見る。
    """
    ev = head()
    ev += [(1.0, bytes([0xc0, 48]))]                 # Strings
    ...
    t = 2.4
    # ビブラート（速さ・深さ・遅れ）
    for lo, val in ((0x15, 90), (0x16, 100), (0x17, 30)):
        ev += [(t, xg([0x08, 0x00, lo, val]))]
        t += 0.05
    ev += note(0, 62, 100, t + 0.2, 1.2)
    t += 1.8
    # 明るさ（18）と響き（19）
    for lo, val in ((0x18, 100), (0x19, 90)):
        ev += [(t, xg([0x08, 0x00, lo, val]))]
        t += 0.05
    ev += note(0, 64, 100, t + 0.2, 1.0)
    t += 1.6
    # 包絡線（立ち上がり・減衰・離し）
    for lo, val in ((0x1a, 40), (0x1b, 90), (0x1c, 100)):
        ev += [(t, xg([0x08, 0x00, lo, val]))]
        t += 0.05
    ev += note(0, 65, 100, t + 0.2, 0.6)
    t += 1.8
    # ベンドの幅を 12 半音にして曲げる
    ev += [(t, xg([0x08, 0x00, 0x23, 64 + 12]))]
    ev += note(0, 60, 100, t + 0.2, 1.2)
    ev += [(t + 0.5, b'\xe0\x00\x60')]
    t += 1.8
    ev += [(t, b'\xe0\x00\x40')]
    # 送り（コーラス・リバーブ・バリエーション）
    for lo, val in ((0x12, 90), (0x13, 20), (0x14, 100)):
        ev += [(t + 0.05, xg([0x08, 0x00, lo, val]))]
        t += 0.05
    ev += note(0, 60, 100, t + 0.3, 1.0)
    return [track(seq(ev))], t + 2.0
